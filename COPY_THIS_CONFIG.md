# 📝 Update config.php on Hostinger

## Why You Are Getting This Error
The error `Call to undefined function getJsonInput()` happens because you updated `projects.php` (which uses this function) but **didn't update `config.php`** (where this function is defined).

Your server has an old version of `config.php`.
...
### 3. Edit config.php
1. Find the file: **config.php** (inside `api` folder)
2. Right-click → **Edit**
...
// 2. Load Secrets
// Try to load from various potential locations to be robust against Hostinger folder structures
$possible_paths = [
    dirname(__DIR__, 2) . '/secrets.php', // Standard: project_root/secrets.php
    dirname(__DIR__) . '/secrets.php',    // Parent of api/ (public_html)
    __DIR__ . '/secrets.php'              // Current dir (fallback)
];
...
Project creation should now work properly.
